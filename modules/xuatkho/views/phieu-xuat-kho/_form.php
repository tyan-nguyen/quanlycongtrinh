...
<?php $form = ActiveForm::begin([
    'action' => '/xuatkho/phieu-xuat-kho/update?id=' . $model->id,
]); ?>

<div class="row">
    <div class="col-xs-12">
        <h5 class="page-header">
            <i class="fa fa-book"></i> Mã phiếu: #<?= $model->soPhieu ?>
...
<div class="row">
	...
	<div class="col-sm-3">
		...
		<a class="btn btn-primary btn-xs" href="/xuatkho/quy-trinh/gui-phieu?idPhieu=<?= $model->id ?>" title="Trình duyệt" role="modal-remote"><i class="glyphicon glyphicon-forward"></i> Gửi phiếu xuất kho</a>
		<?php } ?>
		<?php if($model->trang_thai == 'CHO_DUYET'){ ?>
		<a class="btn btn-primary btn-xs" href="/xuatkho/quy-trinh/duyet-phieu?idPhieu=<?= $model->id ?>" title="Trình duyệt" role="modal-remote"><i class="glyphicon glyphicon-forward"></i> Duyệt phiếu xuất kho</a>
		<a class="btn btn-warning btn-xs" href="/xuatkho/quy-trinh/khong-duyet-phieu?idPhieu=<?= $model->id ?>" title="Không duyệt phiếu" role="modal-remote"><i class="glyphicon glyphicon-remove"></i> Không duyệt phiếu xuất kho</a>
		<?php } ?>
		<?php if($model->trang_thai == 'DA_DUYET'){ ?>
		<a class="btn btn-primary btn-xs" href="/xuatkho/quy-trinh/duyet-giao-hang?idPhieu=<?= $model->id ?>" title="Nhập giao hàng" role="modal-remote"><i class="glyphicon glyphicon-forward"></i> Đặt và giao hàng</a>
		<?php } ?>
